...
const DRAFT_API_BASE_URL = `${SLEEPER_API_BASE_URL}/draft`;
...
export const getLeagueMatchupsAndScores = async () => {
  ...
  const res = await fetch(`${SLEEPER_API_BASE_URL}/league/1004576672147968000/rosters`, { compress: true }).catch((err) => { console.error(err); });
...
export const getUpcomingDraft = async () => {
  ...
  try {
    ...
    const [officialDraftRes, picksRes] = await waitForAll(
      fetch(`${DRAFT_API_BASE_URL}/${draftID}`, { compress: true }),
      fetch(`${SLEEPER_API_BASE_URL}/league/${leagueID}/traded_picks`, {
        compress: true,
      })
    );
    ...
    if (officialDraft.status == "complete") {
      year = year + 1;
      const buildRes = buildFromScratch(
        rosters,
        officialDraft.slot_to_roster_id,
        officialDraft.settings.rounds,
        picks.filter((pick) => parseInt(pick.season) == year),
        regularSeasonLength
      );
    ...
    console.error("Error in getUpcomingDraft:", error);
...
export const getPreviousDrafts = async () => {
  ...
  while (curSeason && curSeason != 0) {
    const [leagueData, completedDraftsInfo] = await waitForAll(
      ...
      fetch(`${SLEEPER_API_BASE_URL}/league/${curSeason}/drafts`, { compress: true })
    ...
    for (const completedDraft of completedDrafts) {
      ...
      const [officialDraftRes, picksRes, playersRes] = await waitForAll(
        fetch(`${DRAFT_API_BASE_URL}/${draftID}`, { compress: true }),
        fetch(`${DRAFT_API_BASE_URL}/${draftID}/traded_picks`, { compress: true }),
        fetch(`${DRAFT_API_BASE_URL}/${draftID}/picks`, { compress: true })
      ).catch((err) => { console.error(err); });
